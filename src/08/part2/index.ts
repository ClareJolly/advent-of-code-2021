const part2 = (inputData: string[]) => {
  ...
  function setLengthSix(mainPattern: string) {
  ...
  function setLengthFive(mainPattern: string) {
    mainPattern.split(' ').forEach(code => {
      if (code.length == 5) {
        if (includeChars(code, codes[1])) {
          codes[3] = code
  ...
  function initialCodes(mainPattern: string) {
    mainPattern.split(' ').forEach(code => {
      switch (code.length) {
        case 2:
          codes[1] = code
  ...
  function isFive(code: string, chars: string) {
    let count = 0
  ...
  function includeChars(code: string, chars: string) {
    let count = 0
  ...
  function exactChars(code: string, chars: string) {
    let count = 0
  ...
  function checkFinalCode(codePattern: string) {
    let codeNums: string[] = []
  ...
  const result = finalCode.map(x => parseInt(x)).reduce((acc, curr) => acc + curr)
